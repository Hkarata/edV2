...
public class RemoveSingleLicenseClassEndpoint (ILicenseClassRepository licenseClassRepository, ILogger<RemoveSingleLicenseClassEndpoint> logger)
    : EndpointWithoutRequest<Results<Ok, Conflict>>
{
    ...
    public override async Task<Results<Ok, Conflict>> ExecuteAsync(CancellationToken ct)
    {
        ...
        logger.LogInformation("Received request to remove license class {LicenseClassId} for user {UserId}.", licenseClassId, userId);
        ...
        try
        {
            ...
            logger.LogInformation("Successfully removed license class {LicenseClassId} for user {UserId}.", licenseClassId, userId);
            return TypedResults.Ok();
        }
        catch (Exception ex)
        {
            // Log any exceptions that occur during the process
            logger.LogError(ex, "An error occurred while removing license class {LicenseClassId} for user {UserId}.", licenseClassId, userId);
